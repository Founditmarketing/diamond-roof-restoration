...
import { Shield, Layers, Maximize, Box, ArrowRight, Check } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {services.map((service, index) => (
            <motion.div
              ...
              className="group p-8 md:p-10 border border-white/10 bg-white/5 hover:bg-white/10 hover:border-cyan/40 transition-all duration-500 cursor-pointer flex flex-col h-full relative overflow-hidden rounded-lg shadow-[0_20px_50px_rgba(0,0,0,0.2)]"
            >
              ...
              <div className="w-14 h-14 bg-black/40 border border-white/10 flex items-center justify-center mb-8 group-hover:border-cyan/50 group-hover:scale-110 transition-all duration-500 relative z-10 rounded">
                <service.icon className="w-6 h-6 text-cyan" />
              </div>
              
              <h3 className="text-2xl font-display font-bold text-white uppercase tracking-wider mb-4 relative z-10">
                {service.title}
              </h3>
              
              <p className="text-ghost/70 text-[14px] leading-relaxed mb-8 flex-grow relative z-10">
                {service.description}
              </p>

              <ul className="space-y-3 mb-8 relative z-10">
                {service.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-3 text-[13px] text-ghost/80">
