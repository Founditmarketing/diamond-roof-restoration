...
export default async function handler(req: any, res: any) {
  ...
  const { source, name, phone, email, address, message, website, company } = req.body ?? {};
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #1a1a2e;">
          <div style="background: linear-gradient(135deg, #0a0f2e 0%, #1a2a4a 100%); padding: 32px; border-radius: 12px 12px 0 0; text-align: center;">
            <h1 style="color: #4091b1; font-size: 24px; margin: 0;">New Lead</h1>
            <p style="color: #a0aec0; margin: 8px 0 0;">Diamond Roof Restoration${source ? ` — ${source}` : ''}</p>
          </div>
          <div style="background: #f7fafc; padding: 32px; border-radius: 0 0 12px 12px; border: 1px solid #e2e8f0;">
            <table style="width: 100%; border-collapse: collapse;">
              ...
              ${address ? `
              <tr>
                <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; font-weight: bold; color: #4a5568;">Address</td>
                <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; color: #2d3748;">${address}</td>
              </tr>` : ''}
              ${message ? `
              <tr>
            ...
            <p style="margin: 24px 0 0; font-size: 12px; color: #a0aec0; text-align: center;">
              Reply directly to this email to respond to ${name}. Sent via diamondroofs.net.
            </p>
          </div>
        </div>
      `,
